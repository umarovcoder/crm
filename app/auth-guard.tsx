...
export default function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  return <><>{children}</><div style={{position:"fixed",right:16,bottom:16,background:"#172b4d",color:"white",padding:"8px 12px",borderRadius:8,fontSize:12,zIndex:50,display:"flex",alignItems:"center",gap:8}}>{user.email} · {role}{role === "super_admin" && <button style={{border:0,borderRadius:6,padding:"4px 7px",cursor:"pointer"}} onClick={() => router.push("/settings")}>⚙ Sozlamalar</button>}<button style={{marginLeft:2}} onClick={() => supabase.auth.signOut().then(() => router.replace("/login"))}>Chiqish</button></div></>;
}
